...
! 201809110836 Wagner- Modificacao Funcao Visco Arrasto 
!##################################################################################################
module ModVVHW
...
!-----------------------------------------------------------------------------  
    ...
    real(8) :: C_peak, Z_peak, S_peak , E_peak
 ...
 !-----------------------------------------------------------------------------       
   ...
   type , extends (ClassConstitutiveModel) :: ClassVVHW
  ...
  !----------------------------------------------------------------------------------------
    ...
    type(VVHWProperties),pointer :: Properties => null()
...
!            procedure :: GetTangentModulus             => GetTangentModulus_VariationalViscoHydrolysis
            procedure :: ConstitutiveModelConstructor  => ConstitutiveModelConstructor_VVHW
            procedure :: ConstitutiveModelDestructor   => ConstitutiveModelDestructor_VVHW
            procedure :: ReadMaterialParameters        => ReadMaterialParameters_VVHW
            procedure :: GetResult                     => GetResult_VVHW
            procedure :: SwitchConvergedState          => SwitchConvergedState_VVHW
            procedure :: CopyProperties                => CopyProperties_VVHW

    end type

    type , extends(ClassVVHW) :: ClassVVHW_3D

    contains
    ! Class Methods
    !----------------------------------------------------------------------------------
    procedure :: UpdateStressAndStateVariables  =>  UpdateStressAndStateVariables_VVHW_3D
    procedure :: GetTangentModulus              =>  GetTangentModulus_VVHW_3D

    end type
    
    type , extends(ClassVVHW) :: ClassVVHW_AXI
    
    contains
    ! Class Methods
    !----------------------------------------------------------------------------------
    procedure :: UpdateStressAndStateVariables  =>  UpdateStressAndStateVariables_VVHW_AXI
    procedure :: GetTangentModulus              =>  GetTangentModulus_VVHW_AXI
...
!******************************************************************************    
    subroutine ConstitutiveModelConstructor_VVHW(this,AnalysisSettings)

    use ModAnalysis

    class(ClassVVHW) :: this
...
!******************************************************************************  
    ...
    ! Date:         Author:
    !==========================================================================================
    subroutine ConstitutiveModelDestructor_VVHW(this)
    ...
    subroutine ReadMaterialParameters_VVHW(this ,DataFile)
    ...
    !************************************************************************************
            ...
            ! Internal variables
            ! ---------------------------------------------------------------------------------
		    character(len=100), dimension(29) :: ListOfOptions, ListOfValues
		    logical, dimension(29)            :: FoundOption
		    integer                          :: i
		    !************************************************************************************
...
!------------------------------------------------------------------------------------
   ...
   ! ListOfOptions(9:13)   = ["s0", "scv", "sg", "sz", "sb"]
    ListOfOptions(9:12)  = ["C_peak", "Z_peak", "S_peak","E_peak"]
    ListOfOptions(13:15) = ["knh", "kHiso", "kcH"]
    ListOfOptions(16)    = "FlagHardening"
    ListOfOptions(17:20) = ["knd", "km" , "kR", "kg"]
    ListOfOptions(21:22) = ["kS", "kN"]
    ListOfOptions(23:25) = ["Threshold", "FlagPlasDam", "FlagHidrDam"] 
    ListOfOptions(26:28) = ["theta", "gamma", "zeta"]
    ListOfOptions(29)    = "alpha_guess"
...
!------------------------------------------------------------------------------------
    ...
    this%Properties% C_peak = ListOfValues(9)
    this%Properties% Z_peak = ListOfValues(10)
    this%Properties% S_peak = ListOfValues(11)
    this%Properties% E_peak = ListOfValues(12)
    this%Properties% knh = ListOfValues(13)
    this%Properties% kHiso = ListOfValues(14)
    this%Properties% kcH = ListOfValues(15)
    this%Properties% FlagHardening = ListOfValues(16)
    this%Properties% knd = ListOfValues(17)
    this%Properties% km = ListOfValues(18)
    this%Properties% kR = ListOfValues(19)
    this%Properties% kg = ListOfValues(20)
    this%Properties% kS = ListOfValues(21)
    this%Properties% kN = ListOfValues(22)
    this%Properties% Threshold   = ListOfValues(23)
    this%Properties% FlagPlasDam = ListOfValues(24)
    this%Properties% FlagHidrDam = ListOfValues(25)
    this%Properties% params(1)   = ListOfValues(26)
    this%Properties% params(2)   = ListOfValues(27)
    this%Properties% params(3)   = ListOfValues(28)
    this%Properties% alpha_guess   = ListOfValues(29)

    end subroutine
!******************************************************************************

!******************************************************************************    
    subroutine CopyProperties_VVHW(this,Reference)
        class(ClassVVHW) :: this
        ...
        select type ( Reference )
            ...
            class default
                stop "ERROR :: CopyProperties :: ClassVVHW - input reference not identified"
        end select
    end subroutine
!******************************************************************************
...
!******************************************************************************     
    ...
    real(8) :: C1, S_0, S_Z, um, C2, C3, u , h, K, S_narrow, S_inf
    ...
    R = alpha 
    S_0 = props%C_peak
    S_narrow = props%S_peak
    S_z = props%Z_peak
    
    S_inf = props%E_peak
    !um = 1.0d0 ! pra nao esquecer que a divisao tem que ser em double :)
    !C2 = 1.0d0 !2.5d0 ! Arbitrário
    !C3 = 1.0d0
    
    K =  S_0 * S_narrow
    
    fArr = S_inf + K * R * exp( - S_z * S_narrow * R ) 
    
    dfArr = K * ( exp( -S_z * S_narrow * R ) + (-S_z * S_narrow) * R * exp( - S_z * S_narrow * R ) )
    
    d2fArr = K * ( 2.0d0 * (- S_z * S_narrow) * exp( - S_z * S_narrow * R ) + ((- S_z * S_narrow )**2.0d0) * R * exp( -S_z * S_narrow * R ) )  
...
!******************************************************************************    
    subroutine UpdateStressAndStateVariables_VVHW_3D(this,Status)
    
    use MathRoutines
        
    class(ClassVVHW_3D) :: this
    type(ClassStatus)  :: Status
...
!==========================================================================    
    ...
    this%Fpn1=Fpn
    !this%vin1=vin
    this%vin1 = [wn1, dpn1, dhn1, alphan1, Yn1, 0, 0, 0, 0, 0]
    this%dWdCiso = dWdC
...
!******************************************************************************     
    ...
    vin1 = [wn1, dpn1, dhn1, alphan1, Yn1, 0, 0, 0, 0, 0]
    ...
    this%Fpn1=Fpn1
    !this%vin1=vin
    this%vin1 = [wn1, dpn1, dhn1, alphan1, Yn1, delta_alpha, fArrn1, 0, 0, 0]
    this%dWdCiso = dWdC
    this%DEV_dWdCiso = DEV_dWdC
    !this%Time_old=Time_old    
...
!******************************************************************************
    ...
    subroutine UpdateStressAndStateVariables_VVHW_AXI(this,Status)
    
    use MathRoutines
        
    class(ClassVVHW_AXI) :: this
    type(ClassStatus)  :: Status
...
!!!************************************************************************************
    ...
    !!!************************************************************************************
    subroutine TanModDF_VVHW_3D(this, Cn1, Cn1_iso, J, Sn1)
    use MathRoutines
...
!******************************************************************************        
    ...
    subroutine GetTangentModulus_VVHW_3D(this,D)
    ...
    real(8) , dimension(:,:) , intent(inout) :: D
       ...
       call TanModDF_VVHW_3D(this, C_f, Ciso_f, Jf, S_f)
       call TanModDF_VVHW_3D(this, C_b, Ciso_b, Jb, S_b)
       ...
       !      ! -----------------------------------------------------------------------------------
       time = this%classVVHW%classconstitutivemodel%time
       D=D
       !write(*,*)
       !do k=1,6
...
!******************************************************************************        
    subroutine TanModDF_VVHW_AXI(this, Cn1, Cn1_iso, J, Sn1)
    use MathRoutines
...
!******************************************************************************        
    subroutine GetTangentModulus_VVHW_AXI(this,D)
    ...
    real(8) , dimension(:,:) , intent(inout) :: D
       ...
       call TanModDF_VVHW_AXI(this, C_f, Ciso_f, Jf, S_f)
       call TanModDF_VVHW_AXI(this, C_b, Ciso_b, Jb, S_b)
...
!******************************************************************************    
    subroutine SwitchConvergedState_VVHW(this)
        class(ClassVVHW) :: this
        this%Fpn=this%Fpn1
        this%vin=this%vin1
...
!******************************************************************************    
    subroutine GetResult_VVHW( this, ID , Name , Length , Variable , VariableType )
    use MathRoutines
    implicit none
